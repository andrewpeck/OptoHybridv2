...
while {[gets $fid line] != -1} {
    if { [ regexp -all -- $version_pattern $line] } {
        regexp {[0-9]{8}} $line matched
        puts $matched
        set  year  [string range ${matched} 0 3]
        set month  [string range ${matched} 4 5]
        set   day  [string range ${matched} 6 7]
    }
}
close $fid
...
set releasedir     release/${shortname}_${datecode}
...
file copy -force $mcs_filename   ${releasedir}/${shortname}-${datecode}.mcs
file copy -force $bit_filename   ${releasedir}/${shortname}-${datecode}.bit
file copy -force $svf_verify     ${releasedir}/${shortname}-${datecode}_verify.svf
file copy -force $svf_noverify   ${releasedir}/${shortname}-${datecode}_noverify.svf
file copy -force $prm_filename   ${releasedir}/${shortname}-${datecode}.prm
file copy -force $cfi_filename   ${releasedir}/${shortname}-${datecode}.cfi
